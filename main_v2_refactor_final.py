...
def validate_config():
    ...
    if not isinstance(SELECTION_PATTERN, str) or not SELECTION_PATTERN:
        errors.append('SELECTION_PATTERN must be a non-empty string')
    if not isinstance(DRIVE_PREFIXES, list) or not DRIVE_PREFIXES:
        errors.append('DRIVE_PREFIXES must be a non-empty list')
    if errors:
...
def run_self_tests():
    ...
    assert calc_pct_change(110, 100) == 10.0
    assert classify_portfolio_status(1.2, 8.5)[0] == '✅ Str'
    assert classify_portfolio_status(0.2, 3.5)[0] == '👀 Bld'
    assert classify_portfolio_status(-0.2, 1.0)[0] == '🟦 Hold'
    assert classify_portfolio_status(-0.8, 0.5)[0] == '⚠️ Weak'
    assert classify_portfolio_status(-3.0, -1.0)[0] == '❌ Bel'
    return True
