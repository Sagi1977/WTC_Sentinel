...
def debug_execution_scan(service):
    ...
    for t, bucket, score in underdogs:
        try:
            ...
            curr_p = float(close_s.iloc[-1])
            day_open = float(open_s.iloc[0])
            _day_chg = ((curr_p / day_open) - 1) * 100
            ...
            if len(sample_errors) < 5:
                sample_errors.append(f"{t}: {str(e)[:60]}")

    report = "🛠 *UnderRadar Debug*\n"
    report += f"`Underdogs total : {total_underdogs}`\n"
    report += f"`No session      : {no_session}`\n"
    report += f"`No price data   : {no_price_data}`\n"
    report += f"`No week open    : {no_week_open}`\n"
    report += f"`Wk% <= 5        : {wk_below_5}`\n"
    report += f"`Added to scan   : {added}`\n"
